-- Fix UUID type casting issue in check_and_consume_quota
-- The error "operator does not exist: uuid = character varying" occurs when comparing UUID with text
...
BEGIN
  ...
  -- Fix: Cast stripe_customer_id to UUID when comparing
  SELECT 
    COALESCE(
      CASE 
...
-- Add comment
COMMENT ON FUNCTION check_and_consume_quota IS 'Atomic quota consumption with proper UUID type casting';