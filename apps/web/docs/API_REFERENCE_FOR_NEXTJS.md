...
**Date**: July 30, 2025  
**Version**: 2.2 (AI Time Billing + R2 Export + Clean Event API for NextJS Team)  
...
### 1. **GET /api/builds/:buildId/events** - Main NextJS API

Get clean, structured build events for a project build.

#### Request
```typescript
const response = await fetch(`${WORKER_BASE_URL}/api/builds/${buildId}/events?userId=${userId}&lastEventId=${lastEventId}`, {
  method: 'GET',
  headers: {
    'x-sheen-signature': generateSignature('', `/api/builds/${buildId}/events`, WORKER_SHARED_SECRET)
  }
});
```
...
### 2. **GET /api/builds/:buildId/status** - Build Status Summary

Get current build status with progress information.

#### Request
```typescript
const response = await fetch(`${WORKER_BASE_URL}/api/builds/${buildId}/status?userId=${userId}`, {
  method: 'GET',
  headers: {
    'x-sheen-signature': generateSignature('', `/api/builds/${buildId}/status`, WORKER_SHARED_SECRET)
  }
});
```
...
**Migration Notes:**
- The `/api/builds/:buildId/events` endpoint returns clean events only
