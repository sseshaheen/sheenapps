...
#### 1. connection
Sent when connection to Claude is established.
```typescript
event: connection
data: {
  "sessionId": "abc-123-def",
  "timestamp": "2024-01-20T10:00:00Z",
  "resumedFrom": "previous-session-id"  // Optional, if resuming
}
```
        ...
        (event) => {
          switch (event.type) {
            case 'connection':
              ...
              if (event.data.resumedFrom) {
                // Show resumption notice
                console.log('Resumed from:', event.data.resumedFrom);
              }
              break;
