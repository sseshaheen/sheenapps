...
1. **API Client Service** (`/src/lib/api/careers-api-client.ts`)
   - Full TypeScript types for all endpoints
   - Error handling with specific error codes
   - File validation and base64 conversion utilities
   - Uses existing `WORKER_BASE_URL` configuration

2. **Translations** (All 9 locales)
   - Complete translation files in `/src/messages/[locale]/careers.json`
   - Supports: en, ar, ar-sa, ar-eg, ar-ae, fr, fr-ma, es, de
   - ICU message format for dynamic content
   - RTL support for Arabic locales

3. **Public Job Listing Page** (`/src/app/[locale]/careers/page.tsx`)
   - Server-side rendering with proper i18n
   - Search functionality with debouncing
   - Filter sidebar (department, type, experience, remote)
   - Pagination with URL state management
   - Responsive grid layout
   - Error handling and loading states

4. **Job Detail Page** (`/src/app/[locale]/careers/[slug]/page.tsx`)
...
5. **Application Form** (`/src/components/careers/application-form.tsx`)
   - Modal overlay with proper accessibility
   - File upload with validation (PDF, DOC, DOCX, <5MB)
   - reCAPTCHA integration (needs site key configuration)
...
2. **Worker API URL**
   - Currently using existing `WORKER_BASE_URL` configuration
   - Ensure worker service has career endpoints deployed
...
├── components/careers/
│   ├── job-listing-content.tsx     # Client component for job list
│   ├── job-card.tsx                 # Individual job card
│   ├── job-filters.tsx             # Filter sidebar
│   ├── job-detail-content.tsx      # Job detail client component
│   └── application-form.tsx        # Application modal form
├── lib/api/
│   └── careers-api-client.ts       # API client service
├── types/
│   └── careers.ts                  # TypeScript types
└── messages/[locale]/
    └── careers.json                 # Translations (9 locales)
```

### 🎨 Implementation Decisions

1. **i18n Pattern**: Following CLAUDE.md guidelines
   - Server-side translation loading
   - Proper locale detection
   - Using next-intl routing

2. **State Management**: URL-based for filters/search
   - Better SEO
   - Shareable URLs
   - Back button support
...
3. **Error Handling**: Comprehensive with fallbacks
   ...
   - 404s redirect to listing
   - Rate limiting with user feedback

4. **Performance**: 
   - SSR for initial load
   - Lazy loading for application form
