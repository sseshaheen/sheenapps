/**
 * Individual Customer API Route
 *
 * GET /api/inhouse/projects/[id]/payments/customers/[customerId] - Get customer
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Call worker to get customer
    const result = await callWorker({
      method: 'GET',
      path: `/v1/inhouse/projects/${projectId}/payments/customers/${customerId}`,
    ...
    return NextResponse.json(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to get customer' } },
      { status: 500 }
    )
  }
}
