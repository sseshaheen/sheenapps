/**
 * Inbox Threads List API Route
 *
 * GET /api/inhouse/projects/[id]/inbox/threads - List threads
 */
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  ...
  return withProjectOwner(req, projectId, async ({ userId }) => {
    ...
    const result = await callWorker({
      method: 'GET',
      path: `/v1/inhouse/projects/${projectId}/inbox/threads`,
      queryParams,
      claims: { userId },
    })
    ...
    if (!result.ok) {
      return NextResponse.json(
        { ok: false, error: result.error ?? { code: 'WORKER_ERROR', message: 'Failed to fetch threads' } },
