/**
 * Analytics User Profile API Route
 *
 * GET /api/inhouse/projects/[id]/analytics/users/[userId] - Get user profile
 */
...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Call worker to get user profile
    const result = await callWorker({
      method: 'GET',
      path: `/v1/inhouse/projects/${projectId}/analytics/users/${targetUserId}`,
    ...
    return NextResponse.json(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to get user profile' } },
      { status: 500 }
    )
  }
}
