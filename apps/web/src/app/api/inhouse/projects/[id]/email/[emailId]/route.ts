/**
 * Individual Email API Route
 *
 * GET /api/inhouse/projects/[id]/email/[emailId] - Get email details
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Call worker to get email
    const result = await callWorker({
      method: 'GET',
      path: `/v1/inhouse/projects/${projectId}/email/${emailId}`,
    ...
    return NextResponse.json(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to get email' } },
      { status: 500 }
    )
  }
}
