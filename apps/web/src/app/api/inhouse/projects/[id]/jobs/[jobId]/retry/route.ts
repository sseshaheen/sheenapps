 ...
 * Retry Job API Route
 *
 * POST /api/inhouse/projects/[id]/jobs/[jobId]/retry - Retry a failed job
 */
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Call worker to retry job
    const result = await callWorker({
      method: 'POST',
      path: `/v1/inhouse/jobs/${jobId}/retry`,
    ...
    return NextResponse.json(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to retry job' } },
      { status: 500 }
    )
  }
}
