/**
 * In-House Mode: Data Export (Placeholder)
 *
 * POST /api/inhouse/projects/[id]/exports
 */
...
export async function POST(
  ...
  try {
    ...
    const result = await callWorker({
      method: 'POST',
      path: `/v1/inhouse/projects/${projectId}/exports`,
      body: {}
    })

    if (!result.ok) {
      return NextResponse.json<ApiResponse<never>>(
        { ok: false, error: { code: result.error?.code || 'WORKER_ERROR', message: result.error?.message || 'Failed to start export' } },
    ...
    logger.error('Export placeholder failed', { error: error instanceof Error ? error.message : String(error) })
    return NextResponse.json<ApiResponse<never>>(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to start export' } },
      { status: 500 }
    )
  }
}
