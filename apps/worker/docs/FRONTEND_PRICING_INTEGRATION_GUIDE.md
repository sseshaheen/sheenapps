...
**Endpoint**: `GET /v1/billing/catalog?currency=USD`

> ⚠️ **Important**: This is a backend API endpoint that requires the Fastify backend server to be running (not just the Next.js frontend).
...
### If API Returns Old Format (Missing New Fields)
...
```json
{
  "key": "starter",
  "price": 19,
  // Missing: monthlyPrice, yearlyPrice, displayedDiscount  
}
```

**Solutions**:
1. **Check Endpoint**: Use `/v1/billing/catalog` (not `/api/pricing/catalog`)
2. **Backend Server**: Ensure Fastify backend server is running (separate from Next.js)
3. **Migration**: Confirm database migration `085_add_yearly_pricing.sql` was executed
4. **Server Logs**: Check backend logs for pricing service errors
...
**Solution**: Verify backend server is running and accessible on its configured port.
