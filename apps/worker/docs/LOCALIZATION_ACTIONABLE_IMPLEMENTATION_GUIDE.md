...
- ❌ Monolithic files (need namespace splitting)
...
### Week 3: Simplified Worker Implementation

#### Day 1: Install Dependencies & Setup

```bash
cd worker
npm install @formatjs/cli intl-messageformat fastify-plugin zod
npm install --save-dev @types/intl-messageformat
```

#### Day 2: Create Fastify i18n Plugin

**File: `plugins/i18n.ts`**
```typescript
...
import { createFormatter } from '../src/i18n/formatter'
...
export default fp(async (app) => {
  ...
  app.addHook('onRequest', async (req) => {
    // Strict locale negotiation (never trust user input directly)
    const locale = resolveLocale(
      req.headers['x-sheen-locale'] as string,     // 1. Explicit header
      req.cookies?.locale,                          // 2. Cookie
      req.headers['accept-language'] as string      // 3. Accept-Language
    )
