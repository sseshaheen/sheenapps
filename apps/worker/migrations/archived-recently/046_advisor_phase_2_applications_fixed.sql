-- =====================================================
-- Migration 046 FIX: Advisor Phase 2 Application System
-- =====================================================
...
-- =====================================================
-- FIXED: RLS policy for advisor profile updates
-- =====================================================
-- CRITICAL FIX: RLS policies cannot use NEW/OLD keywords
-- Instead, we use a trigger-based approach for admin-controlled fields

-- First, create a trigger function to prevent unauthorized changes
CREATE OR REPLACE FUNCTION prevent_advisor_admin_field_changes()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS tr_advisors_protect_admin_fields ON advisors;

-- Create trigger to protect admin fields
CREATE TRIGGER tr_advisors_protect_admin_fields
  BEFORE UPDATE ON advisors
  FOR EACH ROW
  EXECUTE FUNCTION prevent_advisor_admin_field_changes();

-- Simplified RLS policy for advisors (users can only access their own records)
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'advisors' AND policyname = 'advisors_user_select') THEN
    CREATE POLICY advisors_user_select ON advisors
      FOR SELECT USING (user_id = auth.uid());
    RAISE NOTICE '✅ Created RLS policy: advisors_user_select';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'advisors' AND policyname = 'advisors_user_update') THEN
    CREATE POLICY advisors_user_update ON advisors
      FOR UPDATE USING (user_id = auth.uid());
    RAISE NOTICE '✅ Created RLS policy: advisors_user_update';
  END IF;
END $$;
...
-- Fix the constraint reference to use the actual constraint name
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'worker_db_role') THEN
    ...
    GRANT EXECUTE ON FUNCTION prevent_advisor_admin_field_changes() TO worker_db_role;
...
DO $$
BEGIN
  RAISE NOTICE '🎯 Migration 046 FIXED completed successfully: Advisor Phase 2 Application System';
  ...
  RAISE NOTICE '🔒 RLS policies updated with trigger-based protection for admin fields';
  ...
  RAISE NOTICE '✅ FIXED: RLS policy issue - replaced NEW/OLD with trigger-based approach';
END $$;