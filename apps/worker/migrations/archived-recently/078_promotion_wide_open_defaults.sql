-- =====================================================
-- Migration 073: Wide-Open Promotion Defaults
-- =====================================================
-- Purpose: Update all promotions to work with ALL providers by default
-- Author: Claude
-- Date: September 2, 2025
-- =====================================================

BEGIN;

-- Update ALL existing promotions to support ALL providers
-- This makes promotions work everywhere without configuration
UPDATE promotions 
SET 
  supported_providers = ARRAY['stripe', 'fawry', 'paymob', 'stcpay', 'paytabs']::payment_provider_key[],
  supported_currencies = ARRAY['USD', 'EUR', 'GBP', 'EGP', 'SAR']::TEXT[]
WHERE supported_providers IS NULL 
   OR supported_providers = ARRAY['stripe']::payment_provider_key[]; -- Also update Stripe-only promotions

-- Update the function to include Stripe everywhere PLUS regional providers
CREATE OR REPLACE FUNCTION get_preferred_provider_for_region(
  p_promotion_id UUID,
  p_region_code TEXT
) RETURNS payment_provider_key[] AS $$
DECLARE
  v_providers payment_provider_key[];
  v_supported payment_provider_key[];
BEGIN
  -- Get the promotion's supported providers
  SELECT supported_providers INTO v_supported
  FROM promotions
  WHERE id = p_promotion_id;
  
  -- Get regional preferences
  SELECT preferred_providers INTO v_providers
  FROM promotion_regional_config
  WHERE promotion_id = p_promotion_id 
    AND region_code = p_region_code;
  
  IF v_providers IS NULL THEN
    -- Return Stripe PLUS regional providers (Stripe first for familiarity)
    -- Handle both uppercase and lowercase for compatibility
    CASE UPPER(p_region_code)
      WHEN 'EG' THEN 
        v_providers := ARRAY['stripe', 'paymob', 'fawry']::payment_provider_key[];
      WHEN 'SA' THEN 
        v_providers := ARRAY['stripe', 'paytabs', 'stcpay']::payment_provider_key[];
      ELSE 
        v_providers := ARRAY['stripe']::payment_provider_key[];
    END CASE;
  END IF;
  
  -- Intersect with supported providers to ensure we only return valid options
  IF v_supported IS NOT NULL THEN
    RETURN (
      SELECT ARRAY(
        SELECT unnest(v_providers) 
        INTERSECT 
        SELECT unnest(v_supported)
      )
    );
  ELSE
    RETURN v_providers;
  END IF;
END;
$$ LANGUAGE plpgsql;

-- Add comment explaining the approach
COMMENT ON FUNCTION get_preferred_provider_for_region IS 
'Returns preferred payment providers for a region. Always includes Stripe as a global option plus regional providers. Egypt gets Stripe+Fawry+Paymob, Saudi gets Stripe+STC+PayTabs, others get Stripe only.';

COMMIT;