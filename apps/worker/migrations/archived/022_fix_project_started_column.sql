-- Migration 022: Fix project_started column in update_project_metrics_summary function
-- Date: July 27, 2025
-- Purpose: Add missing project_started column to stored procedure to fix NOT NULL constraint violation

BEGIN;

CREATE OR REPLACE FUNCTION public.update_project_metrics_summary(v_build_id character varying)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  SELECT
    ...
    v_user_id,
    v_date,
    v_project_started, -- Add the missing project_started value
    -- Build counts
  ...
  DO UPDATE SET
    project_started = EXCLUDED.project_started, -- Update this too in case it changes
